...
def extract_checksum_for_asset(checksum_text: str, asset_name: str) -> Optional[str]:
    ...
    # Fallback for PowerShell Get-FileHash output, even if wrapped
    # Strips all whitespace to combat line-wraps inside paths.
    import re
    clean_text = re.sub(r'\s+', '', checksum_text)
...
def verify_downloaded_asset(
    ...
    if not url:
        return None
    file_name = asset.get("name") or os.path.basename(url)
