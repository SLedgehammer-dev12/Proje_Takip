...
import fitz  # PyMuPDF
from PySide6.QtWidgets import QScrollArea, QTreeWidget, QTreeWidgetItem
...
class WatermarkOverlay(QScrollArea):
    ...
    def __init__(
        self, parent=None, image_path: Optional[str] = None, opacity: float = 0.08
    ):
        super().__init__(parent)
    ...
    def paintEvent(self, event):
        ...
        if self._pixmap and not self._pixmap.isNull():
            # Scale watermark to ~35% of the shorter side
            target_max = int(min(rect.width(), rect.height()) * 0.35)
            if target_max <= 0:
                return
            pm = self._pixmap
