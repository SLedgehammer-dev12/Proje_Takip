...
from utils import dosyadan_tarih_sayi_cikar, dosyadan_proje_bilgisi_cikar
...
class AnaPencere(QMainWindow):
    ...
    def _trigger_preview_update(self):
        ...
        try:
            ...
            yazi_turu = secili_revizyon.yazi_turu
            yazi_no = None
            if yazi_turu == "gelen":
                yazi_no = secili_revizyon.gelen_yazi_no
            elif yazi_turu == "giden":
                if secili_revizyon.onay_yazi_no:
                    yazi_no = secili_revizyon.onay_yazi_no
                elif secili_revizyon.red_yazi_no:
                    yazi_no = secili_revizyon.red_yazi_no

            if hasattr(self, "yazi_onizleme_etiketi"):
                self.yazi_ac_btn.setEnabled(False)
                self._current_yazi_payload = None

                if yazi_no and yazi_turu != "yok":
                    self.yazi_onizleme_etiketi.setText("Yazı ön izlemesi yükleniyor...")
                    yazi_dok = self.db.yazi_dokumani_getir(yazi_no)
    ...
    def _on_yazi_image_ready(self, image: QImage, yazi_no: str):
        ...
        pixmap = QPixmap.fromImage(image)
        letter_payload = self._build_preview_letter_payload(yazi_no)
        if self.preview_state:
            self.preview_state.show_letter_preview(yazi_no, pixmap, letter_payload)
            return
    ...
    def _arama_kutusu_degisti(self):
        try:
            if self.sekme_widget.currentIndex() == 0:
            ...
            if self.sekme_widget.currentIndex() == 2:
                self.guncelle_gosterge_panelini()
        except Exception as e:
