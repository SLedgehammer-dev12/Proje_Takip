...
def setup_ui(self):
    ...
    self.ana_bolunmus_pencere.addWidget(
        _wrap_main_panel(self, _setup_projeler_panel(self), "projects")
    )
    self.ana_bolunmus_pencere.addWidget(
        _wrap_main_panel(self, self._setup_revizyonlar_panel(), "revisions")
    )
...
def _wrap_main_panel(self, widget, area_name: str):
    if not ENABLE_WATERMARK:
        return widget

    alignments = {
        "projects": Qt.AlignBottom | Qt.AlignRight,
        "revisions": Qt.AlignBottom | Qt.AlignRight,
        "details": Qt.AlignBottom | Qt.AlignRight,
    }
    scales = {
        "projects": PANEL_WATERMARK_SCALE,
        "revisions": max(0.14, PANEL_WATERMARK_SCALE - 0.02),
        "details": PANEL_WATERMARK_SCALE,
    }

    return WatermarkedPanelContainer(
        widget,
        image_path=WATERMARK_IMAGE_PATH,
        opacity=PANEL_WATERMARK_OPACITY,
        scale_ratio=scales.get(area_name, PANEL_WATERMARK_SCALE),
        alignment=alignments.get(area_name, Qt.AlignBottom | Qt.AlignRight),
        margin=20,
    )
